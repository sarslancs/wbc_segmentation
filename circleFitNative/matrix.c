...
MATRIX inverseMatrix(MATRIX *M)
{
	int i,j;
	MATRIX res;
	res = allocateMatrix(M->row, M->column);
	for (i=0; i<M->row; i++)
	{
		for (j=0; j<M->column; ++j)
		{
...
MATRIX xorMatrix(MATRIX *M1, MATRIX *M2)
{
	int i,j;
	MATRIX res;
	res = allocateMatrix(M1->row, M1->column);
	for (i=0; i<M1->row; i++)
	{
		for (j=0; j<M1->column; ++j)
		{
			if ((M1->data[i][j] > 0 && M2->data[i][j] > 0) || (M1->data[i][j] == 0 && M2->data[i][j] == 0))
				res.data[i][j] = 0;
			else
				res.data[i][j] = 1;
		}
...
void or2Matrix(MATRIX *M1, MATRIX *M2)
{
	int i,j;
	for (i=0; i<M1->row; i++)
	{
...
MATRIX bwMATRIX(MATRIX *M, double th)
{
	int i,j;
	MATRIX res;
	res = allocateMatrix(M->row, M->column);
...
MATRIX extractLabel(MATRIX *M, double label)
{
	int i,j;
	MATRIX res;
	res = allocateMatrix(M->row, M->column);
