...
%------------------------------------------------------------------------
function [seg, err] = region_based_active_contours(I,init_mask,control_mask,max_its,alpha,display)
  err = 0;
  iksor = xor(init_mask, control_mask);
  prevIksor = numel(iksor(iksor == 1));
  ...
  for its = 1:max_its   % Note: no automatic convergence test
    ...
    if(mod(its,50) == 0) 
      iksor = xor(phi<=0, control_mask);
      newIksor = numel(iksor(iksor == 1));
      fprintf('.');
      if abs(newIksor - prevIksor) < 50;
          break;
      end
      prevIksor = newIksor;
    end
